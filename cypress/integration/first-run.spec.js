describe('Lokmanya Mess - Stability E2E Test Suite', () => {
  ...
  it('performs full application workflow: first-run PIN setup, login, member creation, settings, CSV export, and factory reset', () => {
    ...
    cy.get('[data-testid="first-run-modal-card"]').should('be.visible');
    cy.get('[data-testid="first-run-pin-input"]').type('999999');
    cy.contains('Save PIN').click();

    // 3. Login pin screen should now be active
    cy.get('[data-testid="login-pin-screen"]').should('be.visible');
    
    // Attempt incorrect login PIN
    cy.get('[data-testid="login-pin-input"]').type('111111{enter}');
    cy.get('.toast-notification').should('be.visible');
    
    // Login with correct PIN
    cy.get('[data-testid="login-pin-input"]').clear().type('999999{enter}');
    ...
    cy.get('[data-testid="customer-fee-input"]').clear().type('3000');
    cy.get('[data-testid="customer-deposited-input"]').clear().type('2500');
    ...
    cy.get('[data-testid="settings-upi-input"]').clear().type('test@upi').blur();

    // Configure archive passcode (4-digit security passcode) using Option 2 (Reset with Owner PIN)
    cy.get('[data-testid="settings-archive-owner-pin-input"]').clear().type('999999');
    cy.get('[data-testid="settings-archive-new-input2"]').clear().type('4444');
    cy.contains('Reset Passcode').click();
    cy.get('.toast-notification').should('contain', 'successfully');

    // The settings page is now immediately locked. Unlock it using the new passcode
    cy.get('[data-testid="settings-passcode-input"]').type('4444{enter}');
    ...
    cy.get('[data-testid="factory-reset-unlock-input"]').type('1111{enter}');
    cy.get('.toast-notification').should('contain', 'Incorrect');
    
    // Enter correct passcode
    cy.get('[data-testid="factory-reset-unlock-input"]').clear().type('4444{enter}');
    ...
    cy.get('[data-testid="factory-reset-confirm-input"]').type('1111{enter}');
    cy.get('.toast-notification').should('contain', 'Incorrect');

    // Confirm using correct passcode
    cy.get('[data-testid="factory-reset-confirm-input"]').clear().type('4444{enter}');
